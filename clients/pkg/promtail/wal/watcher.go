...
type WriteTo interface {
	StoreSeries(series record.RefSeries)
	Append(entries RefEntries) error
}
...
func (w *Watcher) decodeAndDispatch(b []byte) error {
	...
	for _, s := range rec.Series {
		w.writeTo.StoreSeries(s)
	}

	for _, entries := range rec.RefEntries {
		if err := w.writeTo.Append(entries); err != nil && firstErr == nil {
			firstErr = err
		}
	}
